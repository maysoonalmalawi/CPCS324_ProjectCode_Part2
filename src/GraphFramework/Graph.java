...
//--------------------------------------------------------------------------
    ...
    public boolean isDuplicated(int SourceId, int DestinationId) {
        ...
        for (Edge edge : vertices[SourceId].getLinkedList()) {
            if (edge.getTarget().getLabel() == vertices[DestinationId].getLabel()) {
                return true;
            }
        }
    ...
    public void readGraphFromFile(File filename) throws FileNotFoundException {
        try (Scanner input = new Scanner(filename)) {
            ...
            for (int i = edgeNum; i > 0; i--) {
                String sourceLabelString = input.next();
                int sourceLabel = CharToInt(sourceLabelString);
                Vertex source = createVertex(sourceLabel);
                String targetLabelString = input.next();
                int targetLabel = CharToInt(targetLabelString);
...
//--------------------------------------------------------------------------
    ...
    //--------------------------------------------------------------------------   
// meethod to convert the latter to integer based in ascii code
    public int CharToInt(String c) {
        int x = c.charAt(0) - 65;
        return x + 1;
    }
